...
def predictstcgfb():
    model = load_model('stockFB.h5')
    dataset = pd.read_csv("FB.csv", usecols=[1,2,3,4])
    dataset = dataset.reindex(index = dataset.index[::-1])
...
@app.route("/stockpr/MSFT")
def predictstcgms():
    model = load_model('stockMS.h5')
    dataset = pd.read_csv("MSFT.csv", usecols=[1,2,3,4])
    dataset = dataset.reindex(index = dataset.index[::-1])
